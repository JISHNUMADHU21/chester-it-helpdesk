...
export default function EditUserPage() {
  ...
  const { data: userData, isLoading, isError, error: fetchError } = useQuery({
    queryKey: ['admin-user', id],
    queryFn:  () => authAPI.getUser(id).then(r => r.data),
    retry:    false,
  })
  ...
  useEffect(() => {
    if (userData) {
      ...
      setGroupIds(userData.groups?.map(g => g.id) || [])
    }
  }, [userData])
  ...
  const isSelf               = userData?.id === me?.id
  const isTargetAdminOrAbove = ['admin', 'superadmin'].includes(userData?.role)
  ...
  if (isError || !userData) return (
    <div style={{ padding: '28px 32px' }}>
      ...
      <div style={{ background: 'var(--danger-bg)', border: '1px solid #FF8F73', borderRadius: 8, padding: '20px 24px', maxWidth: 500 }}>
        <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--danger)', marginBottom: 8 }}>
          Unable to load user
        </div>
        <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 16 }}>
          {fetchError?.response?.status === 404
            ? 'This user was not found or you do not have permission to edit them.'
            : fetchError?.response?.data?.detail || 'An error occurred while loading the user.'}
        </div>
        <button
          onClick={() => navigate('/admin/users')}
          style={{ background: 'var(--brand)', color: '#fff', border: 'none', borderRadius: 4, padding: '8px 18px', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}
        >
    ...
    <div style={{ padding: '28px 32px' }}>
      ...
      {updateMutation.isError && (
        <div style={{ background: 'var(--danger-bg)', border: '1px solid #FF8F73', borderRadius: 4, padding: '10px 14px', fontSize: 13, color: 'var(--danger)', marginBottom: 20 }}>
          ⚠️ {updateMutation.error?.response?.data?.detail || 'Failed to update user.'}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} noValidate autoComplete="off">
        <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 8, padding: '28px 32px', maxWidth: 700 }}>
          ...
          <Field label="Designation / Job Title">
            <input type="text" autoComplete="off" value={designation}
          ...
          {/* Role — only show if allowed to change */}
          {(isSuperAdmin || (!isTargetAdminOrAbove && !isSelf)) && (
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              {[
                ...
                <label key={String(opt.val)} style={{
                  ...
                  <input type="radio" name="isActive" checked={isActive === opt.val}
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
            ...
            <button type="submit" disabled={updateMutation.isPending}
              style={{ background: 'var(--brand)', color: '#fff', border: 'none', borderRadius: 4, padding: '9px 24px', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', opacity: updateMutation.isPending ? 0.7 : 1 }}
              onMouseEnter={e => { if (!updateMutation.isPending) e.currentTarget.style.background = 'var(--brand-hover)' }}

...
function GroupDropdown({ groups, selectedGroup, onSelect, placeholder = 'All Components' }) {
...
export default function ComponentManagePage() {
  ...
  const { data: componentsData, isLoading } = useQuery({
    queryKey: ['config-components', selectedGroup?.id],
    queryFn:  () => selectedGroup
      ? componentAPI.listByGroup(selectedGroup.id).then(r => r.data.results || r.data)
      : componentAPI.list().then(r => r.data.results || r.data),
  })
  const components = componentsData || []

  const saveMutation = useMutation({
    mutationFn: (data) => selected
      ? componentAPI.update(selected.id, data)
      : componentAPI.create(data),
    onSuccess: () => { queryClient.invalidateQueries(['config-components']); closeModal() },
  })

  const deleteMutation = useMutation({
    mutationFn: (id) => componentAPI.delete(id),
    onSuccess: () => { queryClient.invalidateQueries(['config-components']); setDeleteTarget(null) },
  })

  function openCreate() {
    setSelected(null)
    setName(''); setSlug(''); setDescription(''); setIsActive(true)
    // Managers must default to their own group(s), never empty ("All Groups")
    setGroupIds(selectedGroup ? [selectedGroup.id] : (isManagerOnly ? myGroupIds : []))
    setErrors({})
    setModal('form')
  }
  ...
  function validate() {
    ...
    if (isManagerOnly && groupIds.length === 0) {
      e.groups = 'Please select at least one of your groups. "All Groups" is reserved for admins.'
    }
    setErrors(e)
  ...
  const defaultLabel = (isManagerOnly && visibleGroups.length === 1)
    ? visibleGroups[0].name + ' — Components'
    : 'All Components'
  ...
  return (
    <div style={{ padding: '28px 32px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 4 }}>Admin › Component Management</div>
          <h1 style={{ fontSize: 20, fontWeight: 700, color: 'var(--text)', margin: 0 }}>Component Management</h1>
        </div>
      ...
      <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
        <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 14, fontWeight: 600 }}>
            {selectedGroup ? `${selectedGroup.name} — Components` : defaultLabel}
          </span>
        ...
        {isLoading ? (
          ...
          <div style={{ textAlign: 'center', padding: '48px 20px', color: 'var(--text-muted)' }}>
            ...
            <p style={{ fontSize: 14, fontWeight: 600, marginBottom: 4 }}>No components yet</p>
            <p style={{ fontSize: 13 }}>
              {selectedGroup ? `No components assigned to ${selectedGroup.name}.` : 'Create your first component above.'}
            </p>
          </div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: 'var(--surface-2)' }}>
                {['Component', 'Slug', 'Groups', 'Description', 'Status', 'Actions'].map(h => (
            ...
            <tbody>
              {components.map(item => {
                const canManage = isAdmin || isSuperAdmin || (
                  ...
                  item.groups.every(g => myGroupIds.includes(g.id))
                )
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: '13px 18px' }}>
                      {canManage ? (
                        ...
                        <span style={{ fontSize: 12, color: 'var(--text-faint)', fontStyle: 'italic' }}>View only</span>
                      )}
                    </td>
                  </tr>
                )
      ...
      {/* Create / Edit modal */}
      {modal === 'form' && (
        <Modal title={selected ? 'Edit Component' : 'Create Component'} onClose={closeModal}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <textarea value={description} onChange={e => setDescription(e.target.value)}
              placeholder="Describe when this component is used" rows={3}
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            />
            {errors.groups && <div style={{ fontSize: 12, color: 'var(--danger)', marginTop: 6 }}>⊘ {errors.groups}</div>}
          </div>
      ...
      {deleteTarget && (
        <>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', background: '#fff', borderRadius: 8, padding: '28px 32px', zIndex: 201, width: 440, boxShadow: '0 8px 32px rgba(9,30,66,.3)' }}>
            <h3 style={{ fontSize: 16, fontWeight: 700, margin: '0 0 8px' }}>Delete Component</h3>

...
export default function MyTicketsPage() {
  ...
  const open      = allTickets.filter(t => ['pending', 'in_progress', 'escalated', 'reopened'].includes(t.status)).length
  ...
  return (
    <div style={{ padding: '28px 32px', overflowY: 'auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <h1 style={{ fontSize: 20, fontWeight: 700, color: 'var(--text)', margin: 0 }}>My Tickets</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
        {[
          { label: 'Total',     value: total,     color: 'var(--text)', sub: 'All time'        },
          { label: 'Open',      value: open,      color: '#0747A6',     sub: 'In progress'     },
          { label: 'Resolved',  value: resolved,  color: '#006644',     sub: 'All time'        },
          { label: 'Escalated', value: escalated, color: '#974F0C',     sub: 'Needs attention' },
        ].map(c => (
      ...
      <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderBottom: '1px solid var(--border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <h2 style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)', margin: 0 }}>All My Tickets</h2>
        ...
        {isLoading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px 20px' }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>
              {hasActiveFilters ? 'Try adjusting your filters.' : 'Raise your first request to get started.'}
            </p>
          </div>
        ) : (
